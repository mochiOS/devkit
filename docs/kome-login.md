...
表示するURLには公開`code`だけを含めます。device code、access token、refresh credentialを
...
成功後にAccount情報とDeveloper membershipを取得し、CLI refresh credentialを安全な
credential storeへ保存します。Developer IDは公開識別子であり、秘密として扱いません。
